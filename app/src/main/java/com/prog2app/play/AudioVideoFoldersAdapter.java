...
public class AudioVideoFoldersAdapter extends RecyclerView.Adapter<AudioVideoFoldersAdapter.ViewHolder> {
    ...
    public void setValues(ArrayList<String> folderName, ArrayList<Integer> totalVideo, Activity activity, NativeTemplateStyle styles, UnifiedNativeAd unifiedNativeAd) {
        this.folderName=folderName;
        this.totalVideo=totalVideo;
        this.activity=activity;
        this.styles=styles;
    ...
    @NonNull
    @Override
    public AudioVideoFoldersAdapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = null;
        if(viewType == 1){
            ...
            view = LayoutInflater.from(parent.getContext()).inflate(linearLayout? R.layout.audio_video_folders_items : R.layout.grid_audio_video_folders_items, parent, false);
            return new ViewHolder(view);
        }
    }

    @Override
    public void onBindViewHolder(@NonNull final AudioVideoFoldersAdapter.ViewHolder holder, final int position) {
        if (getItemViewType(position) == 1) {
            ...
            if(position>4 && styles !=null){
                ...
                if(listType.equals("video")){
                    ...
                    holder.totalVideo.setText(totalVideo.get(position) + " Audios");
                }
                holder.relativeLayout.setOnClickListener(new View.OnClickListener() {
                    @SuppressLint("ResourceType")
                    @Override
